...
def resize_images_to_match(original_frame,next_frame):
    height,width=original_frame.shape[:2]
    next_frame_resized=cv2.resize(next_frame,(width,height))
    return original_frame,next_frame_resized
def turn_image_greyscale(original_frame,next_frame):
    gray_original_frame=cv2.cvtColor(original_frame,cv2.COLOR_BGR2GRAY)
    gray_next_frame=cv2.cvtColor(next_frame,cv2.COLOR_BGR2GRAY)
    return gray_original_frame,gray_next_frame
def calculate_ssim(original_frame, next_frame):
    gray_original_frame,gray_next_frame=turn_image_greyscale(original_frame, next_frame)
    score, diff=ssim(gray_original_frame,gray_next_frame,full=True)     #score -1,1=similar#diff per-pixel SSIM values
    return score
def mask_objects(image,threshold=127):#pixels above the value, are=255=objects, below=0=background
    gray=cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    _, binary=cv2.threshold(gray,threshold, 255, cv2.THRESH_BINARY)#get the binary image, white pixels (255) and the background by black pixels (0).
    binary=cv2.bitwise_not(binary)#swap object=0 and background=255 pixels
    # using kernel does morphological operations=remove noises=better mask
    kernel=np.ones((5,5),np.uint8)
    binary=cv2.morphologyEx(binary, cv2.MORPH_CLOSE,kernel)
    # find contours
    contours,_=cv2.findContours(binary,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE) #stores two end points
    # object mask: empty mask
    mask=np.zeros_like(gray)
    cv2.drawContours(mask, contours,-1,255,-1)#highlights object contours
    # swap
...
def compare_frames1(existing_img_path, recapture_filename):
    ...
    try:
        ...
        original_image,next_image=resize_images_to_match(original_image, next_image)
        mask1=mask_objects(original_image)
        mask2=mask_objects(next_image)
        #mask original frame and extract the background: keeps only the white pixels
        background1=cv2.bitwise_and(original_image, original_image, mask=mask1)
        background2=cv2.bitwise_and(next_image, next_image, mask=mask2)
        "Test 1"
        background1_blurred=cv2.GaussianBlur(background1,(15, 15),0)
        background2_blurred=cv2.GaussianBlur(background2,(15, 15),0)#blurring extent

        "Test 4"
        kernel_size=(15,15)#size of the matrix used for blurring
        background1_blurred=cv2.GaussianBlur(original_image,kernel_size,0)
        background2_blurred=cv2.GaussianBlur(next_image,kernel_size, 0)
        score=calculate_ssim(background1_blurred, background2_blurred)
        print("SSIM Score: "+str(score))
        "Test Angle Shifts"
        gray_original_frame, gray_next_frame=turn_image_greyscale(original_image, next_image)
